...
angular.module('ngFormBuilder').config(['formioComponentsProvider', function (formioComponentsProvider) {
    formioComponentsProvider.register('confirmFinancial', {
        title: 'Confirm Financial',
        template: 'formio/components/confirmFinancial.html',
        settings: {
            input: true,
            tableView: true,
            label: '',
            key: 'confirmFinancial',
            placeholder: '',
            protected: false,
            persistent: true,
            clearOnHide: true,
        ...
        controller: ['$scope', '$uibModal', '$localStorage', 'LookUpMgrtService', 'BaseService', function ($scope, $uibModal, $localStorage, LookUpMgrtService, BaseService) {
            ...
            var ctrl = ['$scope', '$uibModalInstance', '$localStorage', 'BaseService', 'toaster', 'component', 'parent', 'messService', 'LookUpMgrtService', function ($scope, $uibModalInstance, $localStorage, BaseService, toaster, component, parent, messService, LookUpMgrtService) {
                ...
                //获取是否独立核算下拉框
                LookUpMgrtService._getArray({ _method: 'getLookUpItemByKL', _id: 'Common-Whether' }, function (_data) {
            ...
            var onClick = function () {
                $uibModal.open({
                    templateUrl: 'confirmFinancial.html',
                    controller: ctrl,
                    size: 'lg',
                    resolve: {
                        component: function () {
        ...
        viewTemplate: 'formio/componentsView/confirmFinancial.html'
    });
}
]);
angular.module('ngFormBuilder').run(['$templateCache', 'FormioUtils', '$http', function ($templateCache, FormioUtils, $http) {
    var promise = $http({ method: 'GET', url: "admin/vendor/angular/ng-formio/plugins/confirmFinancial.html" });
    promise.then(function (resp) {
        $templateCache.put('formio/components/confirmFinancial.html', FormioUtils.fieldWrap(resp.data));
        $templateCache.put('formio/componentsView/confirmFinancial.html', FormioUtils.fieldWrap(resp.data));
    }, function (resp) {
...
angular.module('ngFormBuilder').config(['formioComponentsProvider', function (formioComponentsProvider) {
    formioComponentsProvider.register('confirmFinancial', {
        ...
        views: [
            {
                name: 'Display',
                template: 'formio/components/confirmFinancial/display.html'
            },
            {
                name: 'Data',
...
angular.module('ngFormBuilder').run(['$templateCache', function ($templateCache) {
    ...
    $templateCache.put('formio/components/confirmFinancial/display.html',
        '<ng-form>'
