...
angular.module('ngFormBuilder').config(['formioComponentsProvider', function (formioComponentsProvider) {
	formioComponentsProvider.register('newfileUploadList', {
		title: 'newfileUploadList',
		template: 'formio/components/newfileUploadList.html',
		settings: {
			input: true,
			tableView: true,
			label: '',
			key: 'newfileUploadList',
			placeholder: '',
			protected: false,
			persistent: true,
			clearOnHide: true,
		...
		controller: ['$scope', '$uibModal', 'toaster', 'messService', 'BaseService', '$q', '$localStorage', 'ViewPDF', function ($scope, $uibModal, toaster, messService, BaseService, $q, $localStorage, ViewPDF) {
			...
			$scope.fileNameChanged = function(event){  
				...
				$scope.$apply(function () {
					...
					$scope.data[$scope.componentId] = []
					_.each($scope.fileList, function(_e) {
						$scope.data[$scope.componentId].push(_e.name)
					})
				});
			...
			function progressFunction(evt) {
				$scope.flag = true;
				var progressBar = document.getElementById('progressBar' + $scope.componentId);
				var percentageDiv = document.getElementById("percentage" + $scope.componentId);
			...
			var attachListByKey = function(){
				...
				BaseService._get({_service:'docMgrtService',_method:'attachListByKey',_id:$scope.path},function(_data){
					...
					$scope.data[$scope.componentId + '_backup'] = _data.data;
			...
			function delFileList(_docName) {
				var index = _.findIndex($scope.data[$scope.componentId + '_backup'], function(_n) { return _n == _docName });
				$scope.data[$scope.componentId + '_backup'].splice(index, 1);
				if ($scope.data[$scope.componentId + '_backup'].length == 0) {
                    $scope.data[$scope.componentId] = null;
                    $scope.data[$scope.componentId + '_backup'] = null;
				}
            }
			//删除
            ...
            function onClick() {
            	//创建上传文件目录
				if(_.isEmpty($scope.data[$scope.componentId + 'path'])){
					BaseService._put({_service:"docMgrtService", _method:'createDir', _id:$scope.description},function(_data){						
						$scope.path = _data.data;
						//如果API没值，绑定API
						$scope.data[$scope.componentId + 'path'] = $scope.path;
						attachListByKey()
					})				
				}else{
					//如果API有值
					$scope.path = $scope.data[$scope.componentId + 'path'];
					attachListByKey();
				}
			};
			setTimeout(function() {
		...
		viewTemplate: 'formio/componentsView/newfileUploadList.html'
	});
}
])

angular.module('ngFormBuilder').run(['$templateCache', 'FormioUtils', '$http', function ($templateCache, FormioUtils, $http) {
	var promise = $http({ method: 'GET', url: "admin/vendor/angular/ng-formio/plugins/newfileUploadList.html" });
	promise.then(function (resp) {
		$templateCache.put('formio/components/newfileUploadList.html', FormioUtils.fieldWrap(resp.data));
		$templateCache.put('formio/componentsView/newfileUploadList.html', FormioUtils.fieldWrap(resp.data));
	}, function (resp) {
...
angular.module('ngFormBuilder').config(['formioComponentsProvider', function (formioComponentsProvider) {
	formioComponentsProvider.register('newfileUploadList', {
		...
		views: [
			{
				name: 'Display',
				template: 'formio/components/newfileUploadList/display.html'
			},
			{
				name: 'Validation',
...
angular.module('ngFormBuilder').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('formio/components/newfileUploadList/display.html',
	'<ng-form>' +
