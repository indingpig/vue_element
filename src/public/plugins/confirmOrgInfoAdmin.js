...
angular.module('ngFormBuilder').config(['formioComponentsProvider', function (formioComponentsProvider) {
    formioComponentsProvider.register('confirmOrgInfoAdmin', {
        title: 'Confirm Branch Info',
        template: 'formio/components/confirmOrgInfoAdmin.html',
        settings: {
            input: true,
            tableView: true,
            label: '',
            key: 'confirmOrgInfoAdmin',
            placeholder: '',
            protected: false,
            persistent: true,
            clearOnHide: true,
        ...
        controller: ['$scope', '$uibModal', '$localStorage', 'LookUpMgrtService', 'BaseService', function ($scope, $uibModal, $localStorage, LookUpMgrtService, BaseService) {
            ...
            var ctrl = ['$scope', '$uibModalInstance', '$localStorage', 'BaseService', 'toaster', 'component', 'parent', 'messService', 'LookUpMgrtService', function ($scope, $uibModalInstance, $localStorage, BaseService, toaster, component, parent, messService, LookUpMgrtService) {
                ...
                $scope.confirm = function() {
                    //校验机构简码是否重复
                    BaseService._put({_service:"branchOperateService", _method:'haveOrganizationCode'},
                    {"organizationCode":$scope.record.organizationCode},
                    function(_data){
                        if(_data.status == "300"){  //简码重复
                            toaster.pop('error', messService.getMess("com.hytera.remind"), _data.message);
                        }else{
                            //clearInterval(timer);
                            parent.data.final = {};
                            parent.data[component.key + "_data"] = $scope.record;
                            // var data = JSON.parse(JSON.stringify($scope.record))
                            _.each($scope.record, function(_e, _i, _list) {
                                parent.data['final'][_i] = _e
                            })
                            // parent.data.final = data
            ...
            var onClick = function () {
                $uibModal.open({
                    templateUrl: 'confirmBranchInfo.html',
                    controller: ctrl,
                    size: 'lg',
                    resolve: {
                        component: function () {
        ...
        viewTemplate: 'formio/componentsView/confirmOrgInfoAdmin.html'
    });
}
]);
angular.module('ngFormBuilder').run(['$templateCache', 'FormioUtils', '$http', function ($templateCache, FormioUtils, $http) {
    var promise = $http({ method: 'GET', url: "admin/vendor/angular/ng-formio/plugins/confirmOrgInfoAdmin.html" });
    promise.then(function (resp) {
        $templateCache.put('formio/components/confirmOrgInfoAdmin.html', FormioUtils.fieldWrap(resp.data));
        $templateCache.put('formio/componentsView/confirmOrgInfoAdmin.html', FormioUtils.fieldWrap(resp.data));
    }, function (resp) {
...
angular.module('ngFormBuilder').config(['formioComponentsProvider', function (formioComponentsProvider) {
    formioComponentsProvider.register('confirmOrgInfoAdmin', {
        ...
        views: [
            {
                name: 'Display',
                template: 'formio/components/confirmOrgInfoAdmin/display.html'
            },
            {
                name: 'Data',
...
angular.module('ngFormBuilder').run(['$templateCache', function ($templateCache) {
    ...
    $templateCache.put('formio/components/confirmOrgInfoAdmin/display.html',
        '<ng-form>'
